  ...
  return (
    <div className="px-4 pt-10">
      ...
      <div className="space-y-4 animate-fade-up delay-2">
        {halls.map(hall => (
          <div key={hall.id}>
            <button onClick={() => toggle(hall.id)} aria-label={`${expanded[hall.id] ? 'Collapse' : 'Expand'} ${hall.name}`}
              ...
              <div className="w-11 h-11 rounded-2xl flex items-center justify-center text-2xl shrink-0"
                   style={{ backgroundColor: hall.bgColor }}>{hall.emoji}</div>
              ...
              <span className={`text-stone-400 text-sm transition-transform duration-200 ${expanded[hall.id] ? 'rotate-180' : ''}`}>▾</span>
            </button>
            {expanded[hall.id] && (
              <div className="mt-2 space-y-2 pl-2">
                {hall.items.map((item, i) => (
                  <div key={item.id} className="animate-fade-up" style={{ animationDelay: `${i * 0.05}s` }}>
                    <MenuItemCard item={item} restaurant={hall.name} />
                  </div>
                ))}
              </div>
            )}
